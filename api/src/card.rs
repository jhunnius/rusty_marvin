...
impl Card {
    ...
    pub fn from_rank_suit(rank: u8, suit: u8) -> Self {
        assert!(rank >= Self::TWO && rank <= Self::ACE, "Invalid rank");
        assert!(suit < Self::NUM_SUITS, "Invalid suit");
        Self((suit << 4) | (rank & 0xF))
    }

    // Constructor from index (0..51)
    pub fn from_index(index: u8) -> Self {
        assert!(index < Self::NUM_CARDS, "Invalid index");
    ...
    pub fn from_string(s: &str) -> Self {
        assert!(s.len() == 2, "Invalid card string");
    ...
    pub fn rank_from_char(rank_char: char) -> u8 {
        match rank_char {
            '2' => Self::TWO,
            '3' => Self::THREE,
            '4' => Self::FOUR,
    ...
    pub fn suit_from_char(suit_char: char) -> u8 {
        match suit_char {
            's' => Self::SPADES,
            'h' => Self::HEARTS,
            'd' => Self::DIAMONDS,
...
impl std::fmt::Display for Card {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if !self.is_valid() {
            ...
            let rank_char = Self::rank_to_char(self.rank());
            let suit_char = Self::suit_to_char(self.suit());
...
mod tests {
    ...
    fn test_card_creation() {
        let card = Card::from_rank_suit(Card::ACE, Card::SPADES);
